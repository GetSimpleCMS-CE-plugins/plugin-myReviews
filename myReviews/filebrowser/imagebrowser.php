...
<head>
	...
	<link rel="stylesheet" type="text/css"
		href="../../../<?php echo $admin; ?>/template/style.php?v=<?php echo GSVERSION; ?>" media="screen" />
	<style>
		.wrapper,
		#maincontent,
		#imageTable {
	...
	<div class="wrapper">
		<div id="maincontent">
			<div class="main" style="border:none;">
				...
				<div class="h5">/ <a
						href="?func=<?php echo $func; ?>&amp;w=<?php echo $w; ?>&amp;h=<?php echo $h; ?>&amp;autoclose=<?php echo $autoclose; ?>">uploads</a>
					/
					<?php
					foreach ($pathParts as $pathPart) {
						if ($pathPart != '') {
							...
							?>
							<a
								href="?path=<?php echo $urlPath; ?>&amp;func=<?php echo $func; ?>&amp;w=<?php echo $w; ?>&amp;h=<?php echo $h; ?>&autoclose=1"><?php echo $pathPart; ?></a>
							/
							<?php
							$urlPath .= '/';
						}
							...
							foreach ((array) $dirsSorted as $upload) {
								...
								<tr class="All">
									<td class="" colspan="5">
										...
										<a href="imagebrowser.php?path=<?php echo $p; ?>&amp;func=<?php echo $func; ?>&amp;w=<?php echo $w; ?>&amp;h=<?php echo $h; ?>&autoclose=1"
											title="<?php echo $upload['name']; ?>"><strong><?php echo $upload['name']; ?></strong></a>
									</td>
								</tr>
								<?php
							}
								...
								<tr class="All images">
									<td>
										<a href="javascript:void(0)"
											title="<?php i18n('SELECT_FILE') . ': ' . htmlspecialchars(@$upload['name']); ?>"
											onclick="<?php echo $onclick; ?>">
											<img style="width:60px;height:60px;object-fit: cover"
												src="<?php echo $SITEURL . 'data/uploads/' . $subPath . $upload['name']; ?>" />
										</a>
									</td>
									<td>
										<a class="primarylink" href="javascript:void(0)"
											title="<?php i18n('SELECT_FILE') . ': ' . htmlspecialchars(@$upload['name']); ?>"
											onclick="<?php echo $onclick; ?>">
										...
										<p>
											<?php if (@$upload['title'])
												echo '<b>' . htmlspecialchars($upload['title']) . '</b><br/>'; ?>
											<?php if (@$upload['tags'])
												echo '<i>' . htmlspecialchars(implode(', ', $upload['tags'])) . '</i><br/>'; ?>
											<?php if (@$upload['description'])
												echo preg_replace('/\r?\n/', '<br/>', htmlspecialchars($upload['description'])); ?>
										</p>
									</td>
									<td style="white-space:nowrap;"><span><?php echo $upload['width']; ?> x
											<?php echo $upload['height']; ?></span></td>
									<td style="width:80px;text-align:right;"><span><?php echo $upload['size']; ?></span></td>
									<?php if (isset($filePerms) && isset($fileOwner['name'])) { ?>
										<td style="width:70px;text-align:right;">
											<span><?php echo $fileOwner['name']; ?>/<?php echo $filePerms; ?></span></td>
									<?php } ?>
									<td style="width:85px;text-align:right;">
										<span><?php echo shtDate($upload['date']); ?></span></td>
								</tr>
								<?php if ($debug)
				...
				<script>
					function submitLink(e) {
						...
						window.opener.document.querySelector('.<?php echo $_GET['input'];?>').value = linkerNew.replace("<?php echo $SITEURL; ?>", '');
						window.close();
					}
