...
// Helper function do renderowania pojedynczej karty opinii
// MUSI BYĆ PRZED myreviewsCarousel()
function renderReviewCard($row, $SITEURL) {
...
function myreviewsCarousel($platform = '', $count = 0, $autoplay = true, $interval = 5000, $itemsPerSlide = 1)
{
    global $SITEURL;

    echo '<link href="' . $SITEURL . 'plugins/myReviews/css/myreviews.css" rel="stylesheet">';
    echo '<link href="' . $SITEURL . 'plugins/myReviews/css/carousel.css" rel="stylesheet">';
    ...
    if ($platform !== '' && $platform !== 'mixed') {
        $query = "SELECT * FROM reviews WHERE platform = '" . $platform . "' ORDER BY RANDOM()";
    }

    if ($platform !== '' && $count !== 0 && $platform !== 'mixed') {
        $query = "SELECT * FROM reviews WHERE platform = '" . $platform . "' ORDER BY RANDOM() LIMIT " . $count;
    }
    ...
    // Dla pojedynczych opinii pozostaw starą logikę
    if ($itemsPerSlide === 1) {
        $slides = array_chunk($reviews, 1);
        $totalSlides = count($slides);
    } else {
        // Dla wielu opinii - twórz sliding window
        if ($totalReviews >= $itemsPerSlide) {
            $reviews = array_merge($reviews, $reviews);
        }
        $totalSlides = $totalReviews;
    }

    $carouselId = 'reviews-carousel-' . uniqid();

    echo '<div class="reviews-carousel-wrapper items-' . $itemsPerSlide . '" id="' . $carouselId . '">';
    echo '<div class="reviews-carousel-container">';
    ...
    if ($itemsPerSlide === 1) {
        foreach ($slides as $slideIndex => $slideReviews) {
            ...
            echo '<div class="carousel-slide-grid">';
            
            foreach ($slideReviews as $row) {
                echo renderReviewCard($row, $SITEURL);
            }
        ...
        for ($i = 0; $i < $totalSlides; $i++) {
            ...
            echo '<div class="carousel-slide' . $activeClass . '" data-slide="' . $i . '">';
            echo '<div class="carousel-slide-grid">';
            
            for ($j = 0; $j < $itemsPerSlide; $j++) {
                $index = ($i + $j) % count($reviews);
                $row = $reviews[$index];
                echo renderReviewCard($row, $SITEURL);
            }
    ...
    // Navigation buttons - tylko desktop
    if ($totalSlides > 1) {
        echo '<button class="carousel-prev carousel-desktop-only" onclick="moveCarousel(\'' . $carouselId . '\', -1)">
                <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="15 18 9 12 15 6"></polyline></svg>
              </button>';
        echo '<button class="carousel-next carousel-desktop-only" onclick="moveCarousel(\'' . $carouselId . '\', 1)">
    ...
    (function() {
        ...
        window.moveCarousel = function(carouselId, direction) {
            if (carouselId !== "' . $carouselId . '") return;
        ...
        function startAutoplay() {
            ' . ($autoplay ? 'autoplayInterval = setInterval(function() {
                moveCarousel("' . $carouselId . '", 1);
            }, ' . $interval . ');' : '') . '
        }

        function resetAutoplay() {
            ' . ($autoplay ? 'clearInterval(autoplayInterval);
            startAutoplay();' : '') . '
        }
